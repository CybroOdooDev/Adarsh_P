...
class PharmaQcResultLine(models.Model):
    ...
    def _compute_status(self):
        """Evaluates the actual test value against defined minimum and maximum limits to automatically flag Out of Specification (OOS) results."""
        for rec in self:
            is_oos = False
    ...
    def _onchange_actual_value(self):
        """Marks the result as entered as soon as an actual value is recorded."""
        self.result_entered = True

    @api.model_create_multi
    def create(self, vals_list):
        """Overrides creation to flag the result as entered if an actual value is provided, and auto-triggers an OOS investigation if limits are exceeded."""
    ...
    def write(self, vals):
        """Overrides write to prevent modification of results outside draft/in-progress states, and auto-triggers an OOS investigation if limits are newly exceeded."""
