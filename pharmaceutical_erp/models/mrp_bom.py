...
class MrpBom(models.Model):
    """Extends the standard Bill of Materials (Formula) with pharma-specific versioning,
    pharmacopoeial reference, and QA approval workflow. Only Approved formulas may be used to
    ...
    def _check_approval_fields(self):
        """Validates that both an approver and an approval date are provided when a formula is approved."""
        for rec in self:
    ...
    def _check_operations_required(self):
        """Ensures that an approved BOM has at least one operation with an Effective SOP."""
        for rec in self:
            if rec.formula_status == 'approved':
                ...
                for op in rec.operation_ids:
                    if not op.sop_id or op.sop_id.status != 'effective':
                        raise ValidationError(_("Cannot approve the formula: Operation '%s' uses an SOP that is not currently Effective.") % op.name)
    ...
    def action_reset_draft(self):
        """Reverts the formula back to Draft status, increments the version, and clears any previous approval records."""
        for rec in self:
            rec.write({
