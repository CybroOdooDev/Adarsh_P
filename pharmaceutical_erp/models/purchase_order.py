...
class PurchaseOrder(models.Model):
    ...
    def _compute_approved_vendor_ids(self):
        """Calculates and assigns the list of vendors that have an 'approved' status in the AVL."""
        for order in self:
    ...
    def _compute_pharma_po_state(self):
        """
        Derive the 4-stage pharma statusbar from the native Odoo state and
        the pharma_approval_state:

        Odoo state     pharma_approval_state    →  pharma_po_state
        ──────────────────────────────────────────────────────────
        draft          draft / rejected         →  rfq
        sent           draft / rejected         →  sent
        draft / sent   waiting_approval         →  waiting_approval
        draft / sent   approved (pre-confirm)   →  waiting_approval
    ...
    def write(self, vals):
        """Overrides write to restrict modifications to pharma approval fields to workflow actions only."""
    ...
    def action_submit_pharma_approval(self):
        ...
        Moves pharma_approval_state: draft → waiting_approval.
        The statusbar advances to 'Waiting for Approval'.
        """
        self._check_group(
            'pharmaceutical_erp.group_pharma_procurement',
    ...
    def action_pharma_approve(self):
        """
        QA Director approves the PO after verifying the vendor is on the AVL.
        Moves pharma_approval_state: waiting_approval → approved.
        Non-approved vendors are blocked here — the order stays in
        'Waiting for Approval' until a valid AVL entry exists.
        Restricted to: group_pharma_qa_director
        """
        self._check_group(
    ...
    def action_pharma_reject(self):
        """
        QA Director rejects the PO. Sends it back — Procurement must revise and resubmit.
        Restricted to: group_pharma_qa_director
        """
        self._check_group(
            'pharmaceutical_erp.group_pharma_qa_director',
    ...
    def action_pharma_reset_draft(self):
        """
        Procurement resets a rejected PO back to draft for revision.
        The statusbar returns to 'RFQ' or 'Sent RFQ' depending on native state.
        """
        self._check_group(
            'pharmaceutical_erp.group_pharma_procurement',
    ...
    def button_confirm(self):
        """
        Block PO confirmation unless pharma_approval_state is 'approved'.
        Then run AVL check as a final safety net before standard Odoo confirmation.
        On success the statusbar advances to 'PO'.
        """
        for order in self:
    ...
    def _check_avl(self):
        ...
        Non-approved vendors are blocked — the order cannot leave
        'Waiting for Approval' and cannot be confirmed into a PO.
        Lines without a product_type_pharma flag are skipped (non-GMP items).
        """
