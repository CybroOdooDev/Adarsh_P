...
class PharmaAVL(models.Model):
    """Approved Vendor List (AVL) — one record per product-vendor pair. Only vendors with status
    'approved' may be selected on a Purchase Order for the linked product."""
    _name = 'pharma.avl'
    ...
    def action_submit_review(self):
        """
        Procurement submits the AVL entry for QA Director review.
    ...
    def action_approve(self):
        """
        QA Director approves the vendor. Auto-fills approval date and approved_by.
    ...
    def action_block(self):
        """
        QA Director blocks the vendor. Can be called from any state.
        Restricted to: group_pharma_qa_director
        """
        self._check_group(
            'pharmaceutical_erp.group_pharma_qa_director',
    ...
    def action_reset(self):
        """
        Reset back to Under Review (for corrections before re-submission).
        Restricted to: group_pharma_qa_director
        """
        self._check_group(
            'pharmaceutical_erp.group_pharma_qa_director',
